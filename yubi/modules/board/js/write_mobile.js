$(function() { 
	console.log("here");
	$(document).find("textarea.toggle_label").on("click",function(){
		$(this).focus();
	});
	...
	$(".link_append > a").on("click",function(){
		...
		obj_wrap.append(html_item);
		$(document).find(".module_write_box .write_contbox .module_t .link_modal").css("display","flex");
	...
	$("#v_btn_confirm").bind("click", function (event) {
		...
		$(document).find("._chk_input").each(function(){			
			...
			/* INPUT radio TYPE */				
				...
				if(!$(':checkbox[name="'+$input.attr("name")+'"]:checked').is(":checked")){
					modal({
						type: "alert", /* option : alert/confirm/prompt/success/warning/info/error/primary */
						//text: $("label[for='"+$input.attr("name")+"']").text()+' 선택은 필수사항입니다.',
						text: $("label[for='"+$input.attr("name")+"']").text()+' 선택은 필수사항입니다.',
	...
	$(document).on("focus", ".module_write_box .write_contbox .module_w .module_textbox textarea, .module_write_box .write_contbox .module_w .module_imagebox .img_altbox input", function(){
		$(".module_w .write_cont, .module_w .write_cont .item, .module_w .write_cont .bottom_util .item_del").removeAttr("style");
	...
	$(".module_write_box .write_contbox .module_w .module_textbox .text_box #textbox_0").on("click", function(){
		$("#textbox_0").focus();
	});
	...
	/*textarea 자동높이조절*/
	$(".module_write_box .cont_write > .write_contbox .module_w .module_textbox textarea").on("keyup", function(){
	...
	/*이미지 정렬*/
	$(".module_imagebox .img_align_box .img_util li a").on("click", function(){
		if($(this).parent().hasClass("align_height") == true){
			$(".module_imagebox .img_align_box").addClass("change_align");
		}else{
			$(".module_imagebox .img_align_box").removeClass("change_align");
		}
		$(".module_imagebox .img_align_box .img_util li").removeClass("on");
		$(this).parent().addClass("on");
	});

	/*input, textarea label숨기기*/
	$(".toggle_label").each(function(){
...
function get_cont_item(type, data){
	...
	switch(type){
		case "text" :
			...
			html += 		'</div>';
			html += 	'</div>';
			html += 	'<div class="bottom_util">';
			html += 		'<div class="move_icon"><span class="icon"></span>이동</div>';
			html += 		'<a href="#none" class="item_del"><span class="text_hidden">선택된 입력박스 삭제</span></a>';
			html += 	'</div>';
			html += '</div>';
			break;
		case "image" :			
