$(function(){
	...
	$(document).on("click","input#search_word_btn",function(){
		...
		var gourl = $("form[name='search_form1']").attr("action")+'#search_word';
	...
	$("#page_scale").on("change",function(){
		var gourl = $("form[name='search_form1']").attr("action")+'#page_scale';
		$("form[name='search_form1']").attr("action",gourl );
	...
	$("a.search_reset").on("click",function(){		
		...
		var gourl = $("form[name='search_form1']").attr("action")+'#search_word';
		$("form[name='search_form1']").attr("action",gourl );
		$("form[name='search_form1']").reset();
		*/		
		var gourl = $("a.search_reset").attr("href").replace("#search_word","")+'#search_word';		
		$("a.search_reset").attr("href",gourl);		
	});
	...
	function get_list_data(type, $this, inputObj, wrapObj, img_path, imgSize, callback){			
		...
		//			console.log( json.list );
					...
					print_list(type, json.list, img_path, imgSize,  function(get_html){
							...
							let patten = /\/([^\/]*)\//;
							if(patten.exec(window.location.pathname)[1]=="eng"){
								$(".board_search_box .left_box > span.total").html('Total '+json.total_count+' cases');
							}else{
		...
		if( data != undefined ){
			...
			if ( data.length > 0 ){
					...
					$.each(data, function(e,i){		
						...
						//console.log( data[e].photo_name );
						
						if( t == "sort" ){
							html += '<div class="item ';
							if(data[e].photo_name == undefined ) { html += 'item_img_none'; }
							html += '">';
							...
							if(data[e].photo_name != undefined ) {
							...
							if(data[e].photo_name != undefined ) {
								html += 			'<div class="thumb_box"><img src="'+img_path+'256x1/'+data[e].photo_name+'" alt="'+data[e].title+'" /></div>';
							}
							html += 		'<div class="cont_box">';
							html += 			'<span class="date">'+data[e].top_start+' ~ '+data[e].top_end+'</span>';	
							html += 			'<h3>'+data[e].title+'</h3>';
