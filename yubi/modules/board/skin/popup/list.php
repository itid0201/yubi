...
//	$navi_parameter = make_GET_parameter($data['navi_parameter'], '&amp;', true);
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		if($_SESSION['user_id']=='kkkjh96'){
			//echo "<pre>"; print_r($buffer['link_url']); exit;
		}
		## 삭제글표시 
	...
	$print_table .=  			'<a href="#none" class="category_tab_btn"  data-type="all" style="left:0px;">전체'.($data['sub_mode']=="all"?'<span class="icon">선택됨</span>':'').'</a>	';
	$print_table .=  			'<div class="tab_cont_wrap">';
	$print_table .=  				'<div class="list_wrap popup_list">';
	$print_table .= $print_table_tr;
	$print_table .=  				'</div>';
