	...
	<div class="module_view_box">
		...
		<div class="contbox">
			...
			/*	## 사진 출력 부분 
				...
				## 배너 노출기간 및 -----
				echo '<ul class="popup_contents_wrap">';
				echo 	'<li><span class="date_title">노출기간 :</span> '.$data['top_start'].' ~ '.$data['top_end'].'</li>';			
				echo 	'<li><span class="link_title">URL</span> : <a href="'.(empty($data['link_url'])?'#none':$data['link_url']).'" '.(empty($data['varchar_2'])?'':' target ="'.$data['varchar_2'].'"').' >'.$data['link_url'].'</a></li>';					
				echo '</ul>';			
						  ...
						  if(!empty($reply['file_list'])) { /* 사진 보기 */
							 foreach($reply['file_list'] as $file1) {
								 if($file1['file_type']=='photo') {									
									echo '<div class="photo_view"><img src="./ybmodule.file'.$_SYSTEM['module_config']['path'].'/'.$_SYSTEM['module_config']['board_id'].(!empty($_SYSTEM['module_config']['contents_img_width_size'])?'/'.$_SYSTEM['module_config']['contents_img_width_size'].'x1':'').'/'.$file1['re_name'].'" alt="'.(!empty($file['title'])?$file1['title']:$file1['original_name']).'" /></div>';	
								 }
							}
						  }
			...
			if( count($data['file_list']) > 0 ){
				 $group_file = ( count($data['file_list']) > 0 )?'<a href="#none" class="all_down">전체(Zip)다운로드</a>':'<a href="#none" class="all_down none">전체(Zip)다운로드</a>';
			?>
