	...
	<div class="module_view_box">
		...
		<div class="contbox">
			<div class="viewbox">
				...
				<?php  				
					if($data['open']=='y'||($data['reg_pin']==$_SYSTEM['myinfo']['my_pin'])||($_SYSTEM['permission']['manage']==true||$damdang==true)) {
...
<div class="approval_board">
    ...
    <form id="step_0" action="#">
		...
		<input type="hidden" name="board_idx" id="board_idx" value="<?php echo $_GET['idx']; ?>" />	
    ...
    <form id="step_1" action="#">
		...
		<div class="btn_box">
			...
			<p class="btn_right"><a href="#none" class="btn_big reset_request" title="<?php echo $data['idx']; ?>"><span>신청상태변경</span></a></p>
		</div>
			...
			$reply='';
		 	if($list['approval_line']=='0') {
				...
				if($_SYSTEM['permission']['manage']==true || $proc['user_id']==$_SYSTEM['myinfo']['user_id']) {
					...
					echo '<div class="answer_write'.(empty($reply)?'_standby':'_text').'">'.(empty($reply)?'<span>답변준비중</span>':$reply).'(aa</div>';
	...
	if(count($data['approval']['list'])==0|| $data['process_2']=='1') {
	 ...
	 foreach($data['approval']['list'] as $list) {
		...
		//	$list['reply'] = preg_replace('/<(p|span|font)([^>]*)>/i','<\1>',$list['reply']);
			...
			echo '<div class="progress_info_list">
					<div class="progress">
					<strong class="tit">진행상태</strong>
					<ol>
						<li'.($list['process']=='reg'?' class="on"':'').'><span>신청</span></li>
						<li'.($list['process']=='accept'?' class="on"':'').'><span>접수</span></li>
						<li'.(($list['process']=='next'||$list['process']=='pass')&&$list['step']==2?' class="on"':'').'><span>부서지정</span></li>
						<li'.($list['process']=='next'&&$list['step']>=3?' class="on"':'').'><span>담당자지정</span></li>
						<li'.($list['process']=='complete'?' class="on"':'').'><span>완료</span></li>
					</ol>	
				</div>
				<div class="answer">
					'.htmlspecialchars_decode($list['reply']).'
...
//<![CDATA[
	...
	*/
	...
	$('.reset_request').click(function() {

		$idx = $(this).attr('title');
		if(confirm('정말로 신청상태로 되돌리시겠습니까?\n\n되돌리시면 다시 접수처리를 해야합니다.')) {
			$.post("/ybscript.io/approval/approval_proc",{
				idx : $idx,
