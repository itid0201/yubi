        ...
        <div class="comment">
          ...
          <div class="comment_input">
            <ul class="member_info">
              ...
              <?php if ($_SYSTEM['module_config']['board_id'] !== 'www_resident_participation') { ?>
                <li class="la_add"><span class="label">주소</span><span
                    class="txt"><?php echo $data['address_1']; ?><?php echo $data['address_2']; ?></span></li>
              <?php } ?>
              <?php if($_SYSTEM['module_config']['board_id'] == 'www_resident_participation') { ?>
                <li><span class="label">이메일</span><span class="txt"><?php echo $data['email']; ?></span></li>
                <li><span class="label">생년월일</span><span class="txt"><?php echo $data['varchar_2']; ?></span></li>
              <?php } ?>
            </ul>
          </div>
        </div>
        ...
        if ($status == '1' && $_SYSTEM['permission']['manage'] == true) { ?>
          <form id="step_0">
            <div class="progress_info_list">
              ...
              <input type="hidden" name="staff_myid"
                     value="<?php echo $_SYSTEM['myinfo']['user_name'] . '(' . $_SYSTEM['myinfo']['user_id'] . ')'; ?>"/>
        ...
        <?php if ($status == '2' && $_SYSTEM['permission']['manage'] == true) {
          ...
          <form id="step_1">
			...
			<p><label for="deadline">답변 처리기한</label> <input type="text" value="<?php /*echo ($data['approval']['deadline']==''?date('Y-m-d',strtotime('+5 days')):$data['approval']['deadline']);*/ ?>" name="deadline" id="deadline" size="10" autocomplete="off" /><input type="button" id="deadline_change" value="변경" /></p>-->
              ...
              <p class="btn_right"><a href="#none" class="btn_big" id="answer"
                                      title="<?php echo $data['approval']['list'][0]['idx']; ?>"><span>즉시 답변 완료</span></a>
              </p>
              <div>
                <div class="search_box ori">
                  ...
                  <fieldset>
                    ...
                    <div id="text_div_staff_id" class="text_div">
                      <div id="search_text_staff_id" class="search_text search_staff"></div>
                    </div>
                    <a href="#none" class="btn_sml append_staff"><span>담당 추가</span></a>
                    <!--<a href="#none" class="btn_sml append_view"><span>보기권한추가</span></a>--> <a href="#none"
                                                                                                  class="btn_sml"
                                                                                                  id="sel_complete"><span>배정 완료</span></a>
                    <!--<a href="#none" class="btn_sml"><span>초기화</span></a>-->
                  </fieldset>
                </div>
                <h4>답변 담당자</h4>
                <div class="staff_list" id="select_staff_list"><span
                    class="blank">&quot;담당추가&quot;를 클릭하여 담당자를 배정해주세요.</span></div>
                <!--<h4>보기 권한자</h4>
      ...
      <?php if ($_SESSION['user_level'] <= 6) { ?>
        ...
        <script type="text/javascript" name="import_jquery.ready">
            ...
            $('#sel_complete').click(function () {
                ...
                if ($('#select_staff_list > .list > li').length < 1) {
                    alert('지정부터 해주세요');
                    return false;
                }
            ...
            $('.sel_pass').click(function () {
                ...
                if ($(this).parents().parents().parents().children('.staff_list').children('li').length < 1) {
                    alert('지정부터 해주세요');
                    return false;
                }
