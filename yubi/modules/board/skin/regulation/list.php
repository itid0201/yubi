...
//		$print_table_tr  .=  '								<td>'.$buffer['visit_cnt'].'</td>';
	...
	//$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 제목, '.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').' 첨부파일, '.$writer_name.', 등록일, 조회수로 구성되어 있습니다.</caption>';
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 제목, '.$writer_name.', 등록일, 상태, 처리부서로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
...
/*		if(!empty($data['list_msg'])) {
		...
		if( $_SYSTEM['hostname']  == 'business' ) {
			...
			<li class="f_red"><strong>[문의하기] 클릭</strong> -> "휴대폰 본인확인 서비스 또는 소셜로그인" 인증 -> 민원등록</li>
			<li>처리 내용을 확인하기 위해서는 문의하기 시 인증한 방법으로 로그인 한 후 해당 민원창구로 이동하여 
			...
			if($data['device']=='mobile') echo '<p>'.stripslashes($data['list_msg']).'</p>';
				...
				if($_SYSTEM['module_config']['skin_style'] != "regulation"){
					echo '<div class="sub-tit-top"></div>';
					echo '<div class="content_top_alert">';
					echo 	'<div class="info">';
					echo 		'<div class="alert_content">'.stripslashes($data['list_msg']).''.$login_text;					
					echo 			'<div class="alert_btn_box mat20">';
									if($_SYSTEM['myinfo']['is_login'] == true) {
										if($_SYSTEM['module_config']['board_id'] != 'www_inconvenience' && $_SYSTEM['module_config']['board_id'] != 'www_reaulatory_reform' && $_SYSTEM['module_config']['board_id'] != 'www_poor' && $_SYSTEM['module_config']['board_id'] != 'www_newspaper' && $_SYSTEM['module_config']['board_id'] != 'www_pollution' && $_SYSTEM['module_config']['board_id'] != 'www_cyber_disaster' ) {
											echo '<a href="?mode=write" class="alert_btn btn_reportform"><span>문의하기</span></a>';					
											echo '<a href="?mode=list" class="alert_btn btn_mywrite"><span>'.($_SYSTEM['permission']['admin']==true?'내가쓴글보기':'내가쓴글보기').'</span></a>';
										}
										if(!empty($data['old_link'])) echo '<a href="'.$data['old_link'].'" class="btst btn1 p2 btn_oldboard" target="_blank" title="새창"><span class="btn1hover1"></span><span>이전게시판보기</span></a>';
									} else {
										if($_SYSTEM['module_config']['board_id'] != 'www_inconvenience' && $_SYSTEM['module_config']['board_id'] != 'www_reaulatory_reform' && $_SYSTEM['module_config']['board_id'] != 'www_poor' && $_SYSTEM['module_config']['board_id'] != 'www_newspaper' && $_SYSTEM['module_config']['board_id'] != 'www_pollution' && $_SYSTEM['module_config']['board_id'] != 'www_cyber_disaster') {
										echo '<a href="'.$_SYSTEM['rep_login'].$_SERVER['PHP_SELF'].'?mode=write" class="alert_btn btn_reportform"><span class="btn1hover1"></span><span>문의하기</span></a>';
										echo '<a href="'.$_SYSTEM['rep_login'].$_SERVER['PHP_SELF'].'?mode=list" class="alert_btn btn_mywrite"><span class="btn1hover1"></span><span>'.($_SYSTEM['permission']['admin']==true?'내가쓴글보기':'내가쓴글보기').'</span></a>';
										}
										if(!empty($data['old_link'])) echo '<a href="'.$data['old_link'].'" class="btst btn1 p2 btn_oldboard" target="_blank" title="새창"><span class="btn1hover1"></span><span>이전게시판보기</span></a>';
									}				
					echo 			'</div>';
					...
					echo 	'</div>';						
					echo '</div>';
					echo '<div class="sub-tit-bottom"></div>';
				}else{
					if($_SYSTEM['module_config']['board_id'] != "www_singo"){
						...
						if($_SYSTEM['myinfo']['is_login'] == true) {
							echo '<a href="?mode=write" class="p2 mar5 btn_reportform"><span>'.$print_btn_text.'</span></a>';
							echo '<a href="?mode=list" class="p3 btn_mywrite"><span>'.($_SYSTEM['permission']['admin']==true? $print_article_text :'내가쓴글보기').'</span></a>';
						}else {
							echo '<a href="'.$_SYSTEM['rep_login'].$_SERVER['PHP_SELF'].'?mode=write" class="p2 mar5 btn_reportform"><span class="btn1hover1"></span><span>'.$print_btn_text.'</span></a>';
							echo '<a href="'.$_SYSTEM['rep_login'].$_SERVER['PHP_SELF'].'?mode=list" class="p3 btn_mywrite"><span class="btn1hover1"></span><span>'.($_SYSTEM['permission']['admin']==true? $print_article_text:'내가쓴글보기').'</span></a>';
						}
						echo 			'</div>';
					}	
				}		
