...
//		$video_info = getDuration( $buffer['link_url'] );
		...
		## 제목 옆에 붙는 아이콘 
		$print_img = $print_h3_class = $print_img_new = '';
		$print_img_cnt = 0;
		...
		if( !empty($buffer['link_url']) ){			
			$img .= '        <img width="321" height="195" src="https://img.youtube.com/vi/'.$buffer['link_url'].'/0.jpg" alt="" class="thumb_img" /> ';
		}else {			
			$img .= '<img src="/images/board/noimage_video.jpg" alt="이미지가 없습니다." />';
		}
		...
		$print_table_tr  .=  '    				<a href="#none" data-idx="'.$buffer['idx'].' " data-link="'.$href_view.'" title="'.$buffer['title'].' 에 대한 글내용 보기." class="item_cont">';
		$print_table_tr  .=  $img;
		if($_SYSTEM['hostname'] != 'njursc') {
		//$print_table_tr  .=  					'<span class="playtime"><span class="icon"></span>'.$video_info['duration'].'</span>';
		}
		$print_table_tr  .=  '    				</a>';
		...
		$print_table_tr  .=  '    							<li class="twitter share_btn"><a href="#none" class="share_btn" data-type="twitter" data-url="'.$href_view.'" data-title="'.$buffer['title'].'"><span class="icon  share_btn">엑스</span></a></li>';
...
/*		$print_table_tr  .=  '    			<div class="cont_box">';
		$print_table_tr  .=  '    				<strong class="title">'.$print_img.$buffer['title'].'</strong>';		
		$print_table_tr  .=  '    				<span class="date">'.$print_img_new.$buffer['reg_date'].'</span>';
		$print_table_tr  .=  '    			</div>';		*/
		$print_table_tr  .=  				'<div class="cont_box">';
		$print_table_tr  .=  				'	<div class="top_util">';
		$print_table_tr  .=  				'		<ul>';
		$print_table_tr  .=  				'			<li class="view"><span class="icon">조회수</span>'.$buffer['visit_cnt'].'</li>';
		if( $buffer['reply_cnt'] > 0 ){
		$print_table_tr  .=  				'			<li class="reply"><span class="icon">댓글수</span>'.$buffer['reply_cnt'].'</li>';	
		}		
		$print_table_tr  .=  				'		</ul>';
		$print_table_tr  .=  				'	</div>';
		$print_table_tr  .=  				'	<h3>'.$print_img.$buffer['title'].'</h3>';
		$print_table_tr  .=  				'	<dl>';
		$print_table_tr  .=  				'		<dt class="text_hidden">작성날짜</dt><dd>'.$buffer['reg_date'].'</dd>';
		$print_table_tr  .=  				'		<dt class="text_hidden">작성자</dt><dd class="name">'.$writer.'</dd>';
		$print_table_tr  .=  				'	</dl>';
		$print_table_tr  .=  				'</div>';
		$print_table_tr  .=  '    		</div>';
	...
	$print_table  = '';	
	$print_table .=  '<div class="board_video board_list">';
...
<link type="text/css" href="/js/jquery/datetimepicker/jquery.datetimepicker.css" rel="stylesheet" >
