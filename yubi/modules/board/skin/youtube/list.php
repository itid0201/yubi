...
/*function getDuration($videoID){
   ...
   $apikey = $_SYSTEM['key']['google_apikey']; // Like this AIcvSyBsLA8znZn-i-aPLWFrsPOlWMkEyVaXAcv
   $dur = file_get_contents("https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=$videoID&key=$apikey");
   $VidData =json_decode($dur, true);
   $VidSize = $VidData['fileDetails']['fileSize'];	
	
   foreach ($VidData['items'] as $vidTime)
   {	      
	   $VidDuration = $vidTime['contentDetails']['duration'];
   }
    preg_match_all('/(\d+)/',$VidDuration,$parts);
    $return['duration'] =  $parts[0][0].":".sprintf("%02d",$parts[0][1]).(!empty($parts[0][2])?":".sprintf("%02d",$parts[0][2]):''); // Return 1:11:46 (i.e) HH:MM:SS
	$return['fileSize'] = $VidSize ;
	
//	print_r( $VidData )	;exit;	
	...
	//	$video_info = getDuration( $buffer['link_url'] );
		...
		$print_table_tr  .=  			'<img src="https://img.youtube.com/vi/'.$buffer['link_url'].'/0.jpg" alt="" />';
...
//		$print_table_tr  .=  			'<span>1080P / '.$video_info['fileSize'].'KB</span>';
		...
		$print_table_tr  .=  			'<dt class="text_hidden">작성자</dt><dd>'.$buffer['reg_name'].'</dd>';
		$print_table_tr  .=  		'</dl>';
		$print_table_tr  .=  	'</div>';
		$print_table_tr  .=  '</div>';
		...
		$print_table_tr  .=  '    					</div>';
		$print_table_tr  .=  '    				</a>';
		$print_table_tr  .=  '    				<a href="#none" class="board_btn_share">게시글 공유하기</a>';
		$print_table_tr  .=  '    			</div>';		*/
	...
	$print_table .=  '					<div class="board_video">';
