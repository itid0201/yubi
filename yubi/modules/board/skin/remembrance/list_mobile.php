	...
	***********/	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr = '<div class="item">
									<a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  title="'.$buffer['title'].'에 대한 글내용 보기." class="chumo">
										<strong>'.($buffer['is_top']=='true' ? $top_img:'').$tag_span.(($buffer['board_id']=='www_notice' && !empty($buffer['depart_name']))?'['.$buffer['depart_name'].'] ':'').$buffer['contents'].'</strong>
										<span class="date">'.($_SYSTEM['module_config']['board_id'] == 'njursc_newsrelease'?date("Y-m-d", strtotime($buffer['varchar_1'])):$buffer['reg_date']).'</span>
									</a>
							</div>';
		$original_cnt++;
