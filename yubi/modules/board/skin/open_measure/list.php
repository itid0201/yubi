<?php	
	...
	//# 내용을 목록으로 불러오므로 스킨 적용시 (글목록>내용자르기) 제목크기로 맞춰줘야함!!
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr  .=  '								<td><a href="'.$href_view.'" '.$link_class.' title="'.$buffer['title'].' 에 대한 글내용 보기." >'.$buffer['title'];
		$print_table_tr  .= $print_img.'</a></td>';			
		$print_table_tr  .=  '								<td class="align_left"><a href="'.$href_view.'" '.$link_class.' title="'.$buffer['title'].' 에 대한 글내용 보기." >';		
		$print_table_tr  .=  $buffer['contents'];
		$print_table_tr  .=  '								</a></td>';
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 상호명, '.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').($data['use_list_attach'] == 'true'?'첨부파일,':'').' 위반내용, 위반일자, 조회수로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	$print_table .=  '									<th scope="col" class="th_150px">상호명</th>';	
	$print_table .=  '									<th scope="col">위반내용</th>';
	$print_table .=  '									<th scope="col" class="th_120px">위반일자</th>';	
