<?php	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr  .=  '<td class="list_cate">'.$buffer['varchar_1'].'</td>';		
	...
	//	$print_table_tr  .=  '								<td>'.$writer.'</td>';
		...
		$print_table_tr  .=  '								<td>'.$buffer['varchar_5'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_6'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_7'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_8'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_9'].'</td>';
		$print_table_tr  .=  '							</tr>';
		$original_cnt++;
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 구분, 사업명칭, 사업승인일, 건축위치, 사업주체, 층수, 동수, 세대수, 비고'.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').($data['use_list_attach'] == 'true'?'첨부파일,':'').'로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	$print_table .=  '									<th scope="col" class="th_100px">구분</th>';
	$print_table .=  '									<th scope="col">사업명칭</th>';
	$print_table .=  '									<th scope="col" class="th_100px">사업승인일</th>';
	$print_table .=  '									<th scope="col" class="th_100px">건축위치</th>';
	$print_table .=  '									<th scope="col" class="th_100px">사업주체</th>';
	$print_table .=  '									<th scope="col" class="th_80px">층수</th>';
	$print_table .=  '									<th scope="col" class="th_80px">동수</th>';
	$print_table .=  '									<th scope="col" class="th_80px">세대수</th>';
	$print_table .=  '									<th scope="col" class="th_100px">비고</th>';
	//$print_table .=  '									<th scope="col" class="th_120px" rowspan="2">'.$writer_name.'</th>';	
