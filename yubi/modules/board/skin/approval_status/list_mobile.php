	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr = '<div class="item">
									<a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  title="'.$buffer['title'].'에 대한 글내용 보기."  '.$link_class.'>
										<strong>['.$buffer['varchar_1'].']'.($buffer['is_top']=='true' ? $top_img:'').$buffer['title'].$print_img.'</strong>
										<span>사업 승인일 : '.$buffer['varchar_2'].'</span>
										<span>건축위치 : '.$buffer['varchar_3'].'</span>
										<span>지번 : '.$buffer['varchar_4'].'</span>
										<span>사업 주체 : '.$buffer['varchar_5'].'</span>
										<span>층수 : '.$buffer['varchar_6'].'</span>
										<span>동수 : '.$buffer['varchar_7'].'</span>
										<span>세대수 : '.$buffer['varchar_8'].'</span>
										<span>비고 : '.$buffer['varchar_9'].'</span>
									</a>
							</div>';
		$original_cnt++;
...
<div class="board_wrapper">
	<div class="module_list_box">
		...
		<div class="board_list_box">
			...
			echo   '<div class="board_list bd_item_list">';				
			echo $print_table; 
			echo   '</div>';						
