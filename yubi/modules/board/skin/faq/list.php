<link rel="stylesheet" href="<?php echo $_SERVER['SELF'] ?>css/faq.css" type="text/css" />
<?php
/* list skin */
...
## 카테고리 사용시 필드 추가
if ( $data[ 'use_category_tab' ] == 'false' ) {
  $colspan++;
}
...
for ( $iter = 0; $iter < $data[ 'count' ]; $iter++ ) {
  ...
  $print_table_tr .= 		'<div class="col">';
  $print_table_tr .=  		'<div class="list_checkbox">
  								<input type="checkbox" name="check_idx[]" id="check_idx_'.$buffer['idx'].'" value="'.$buffer['idx'].'" />
  								<label for="check_idx_'.$buffer['idx'].'">게시글 선택</label>
							</div>';			  
  $print_table_tr .= '</div>';	  
  } 

  
  if($data['use_category_tab'] == 'false') $print_table_tr .= '<div class="col category" >'.$buffer['category_1'].'</div>';				
  $print_table_tr .= '		<div class="col quest" data-id="'.$buffer['idx'].'">';
  $print_table_tr .= '			<span class="icon_que">Question</span>';
  $print_table_tr .= '			<a href="' . $href_view . '" ' . $link_class . ' title="' . $buffer[ 'title' ] . ' 에 대한 글내용 보기." >' . $buffer[ 'title' ];
  //$print_table_tr .= $print_img;
  $print_table_tr .= '			</a>';	
  $print_table_tr .= '		</div>';
  if($_SYSTEM['module_config']['board_id'] != "citizen_if_then") $print_table_tr .= '		<div class="col">조회 : <span>' . $buffer[ 'visit_cnt' ] . '</span></div>';
 ## ---------------------------------------------------------------
  if ( $_SYSTEM[ 'permission' ][ 'admin' ] == true ) {
    $print_table_tr .= ' 	<div class="col">';
    $print_table_tr .= ' 		<a href="' . $_SERVER[ 'PHP_SELF' ] . '?idx=' . $buffer[ 'idx' ] . ( empty( $view_parameter ) ? '' : '&amp;' . $view_parameter ) . '" class="l_btn_modify">수정</a>';
    $print_table_tr .= '	</div>';
  }
  ## ---------------------------------------------------------------	
  $print_table_tr .= '</div>';
	...
	if( $buffer['html_tag'] == "a" ){
		...
		$contents = get_html_contents( $buffer['contents'], $buffer['file_list_file'], "list", $buffer );
	}else{
		$temp[] = nl2br( $buffer['contents'] );
		$contents = get_html_contents( $temp, $buffer['file_list_file'] );
	}
	############################# contetns #############################	

	
  $print_table_tr .= '<div class="answer_box"><span class="text_hidden">답변</span>'.$contents.'</div>';
  $print_table_tr .= '</li> ';
...
if ( empty( $print_table_tr ) ) {
	$print_table_tr = '<div class="no_result"><span class="icon"></span>검색내역이 없습니다.</div>';  
}
...
/*
$print_table .= ' <div class="confirm_head'.($data['use_category_tab'] == 'false'?' viewCate':'').'">';
$print_table .= 	' <ul>';
$print_table .= 		' <li><span>번호</span></li>';
if($data['use_category_tab'] == 'true') $print_table .= '<li class="faq_cate"><span>분류</span></li>';	  
$print_table .= 		' <li><span>질문</span></li>';
if($_SYSTEM['module_config']['board_id'] != "citizen_if_then") $print_table .= ' <li><span>조회수</span></li>';
if ( $_SYSTEM[ 'permission' ][ 'admin' ] == true )	$print_table .= ' <li><span>관리</span></li>';
$print_table .= 	' </ul>';
$print_table .= ' </div>';
*/  
$print_table .= '<div class="confirm_body'.($data['use_category_tab'] == 'false'?' viewCate':'').'">';
$print_table .= '	<ul class="confirm_list">';
...
<div class="board_wrapper">
  ...
  //    $category_parameter = make_GET_parameter( $arr_temp, '&amp;', true);
      echo print_category( $data[ 'category_1' ], $category_1_list, $category_1_all, $arr_temp, $_SERVER[ 'PHP_SELF' ], 'false' );
    }
    ## -----------  카테고리 : end
...
<script>
	...
	$(document).on('click','.quest', function(){
		var $this = $(this);
		var table_idx = $(this).data("id");
		var expire = 60;	
		var cnt = "<?php echo $buffer['visit_cnt']; ?>";
		...
		var param = {table_idx:table_idx, expire:expire};
		console.log( param );
		$.post("/ybscript.io/module/faq/increment_visit",param).done(function(visit){
			console.log( visit );
			if(visit > 0 ){
				$this.next("div").find("span").html(visit);
			}
		});
				
	}).on("mouseover",'.quest', function(){
		$(this).css('cursor','pointer')
	});
		
	/* 20150921 정재윤 : 삭제 시 경고메세지 */	
	$(".l_btn_remove").click(function() {
