...
if( $data['html_tag'] == "a" ){
	...
	$contents = get_html_contents( $temp, $data['file_list'] );
}
############################# contetns #############################
?>
...
<div id="board_basic_view" class="module_view_box">
<div class="view_titlebox">
  <h3><?php echo $data['title']; ?></h3> 
  <dl>
    <dt>글번호</dt>
    <dd><?php echo $data['idx']; ?></dd>
  	<dt>작성일</dt>
...
<div class="board_content">
    ...
    ?>
		...
		if($data['open']=='y'||($data['reg_pin']==$_SYSTEM['myinfo']['my_pin'])||($data['open']=='n'&&($_SYSTEM['permission']['manage']==true||$damdang==true))||$data['view_permission']=='true') {
			echo $contents;
		} else {
...
<h3 class="approval_title">진행상태</h3>
    ...
    <form id="step_1">
        ...
        <div class="search_box">
			...
			<fieldset>
				...
				<div id="text_div_staff_id" class="text_div"><div id="search_text_staff_id"  class="search_text" ></div></div>
				<a href="#none" class="btn_sml append_staff"><span>담당추가</span></a> <a href="#none" class="btn_sml append_view"><span>보기권한추가</span></a> <a href="#none" class="btn_sml" id="sel_complete"><span>배정완료</span></a> <a href="#none" class="btn_sml"><span>초기화</span></a>
		 ...
		 foreach($data['approval']['list'] as $list) {
			...
			$reply='';
		 	if($list['approval_line']=='0') {
				...
				$html=$html0;
				//if($_SESSION['user_id']=='irony75') print_r($data['approval']);
				...
				if($_SYSTEM['permission']['manage']==true || $proc['user_id']==$_SYSTEM['myinfo']['user_id']) {
					echo '<div class="answer_write"><textarea cols="70" rows="10" id="answer_contents_'.$list['approval_line'].'">'.$reply.'</textarea></div>';
					...
					echo ($_SYSTEM['permission']['manage']==true?'<!--<a href="#none" class="btn_sml urging"  title="'.$list['idx'].'"><span>답변촉구요청</span></a>--> <a href="#none" class="btn_sml reset_select"  title="'.$list['idx'].'"><span>접수상태로변경</span></a> <a href="#none" class="btn_sml add_item"  title="'.$list['idx'].'"><span>담당자추가지정</span></a>':'');
					echo '</p>';
	...
	if(count($data['approval']['list'])==0|| $data['process_2']=='1') {
	 ...
	 foreach($data['approval']['list'] as $list) {
		...
		//	$list['reply'] = preg_replace('/<(p|span|font)([^>]*)>/i','<\1>',$list['reply']);
			...
			echo 	'<div class="staff"><span class="date">'.date('Y년m월d일 H시m분',strtotime($list['processing_date'])).'</span><span class="dept">'.str_replace('|',' ',$list['dept_path_title']).'</span><span class="name">'.$list['user_name'].(!empty($list['dept_tel'])?'('.$list['dept_tel'].')':'').'</span></div>
				</div>';
		 }
	}?>
		</div>
