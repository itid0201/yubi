<?php	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr  .=  '								<td class="align_left"><a href="'.$href_view.'" '.$link_class.' title="'.$buffer['title'].' 에 대한 글내용 보기." >'.$buffer['title'];	
//		$print_table_tr  .= $print_img;
		$print_table_tr  .=  '									</a>';
		$print_table_tr  .=  '								</td>';
		//$buffer['varchar_1'] = date("Y-m-d", strtotime($buffer['varchar_1']));
		$print_table_tr  .=  '								<td>'.$buffer['varchar_1'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_2'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['varchar_3'].'</td>';
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 토지소재지, '.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').($data['use_list_attach'] == 'true'?'첨부파일,':'').' 지목(공부상), 면적(㎡), 관리부서로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	if($data['use_category_1'] == 'true') 			$print_table .=  '	<th scope="col" class="th_150px">분류</th>';
	$print_table .=  '									<th scope="col">토지소재지</th>';	
	$print_table .=  '									<th scope="col" class="th_150px">지목(공부상)</th>';	
	$print_table .=  '									<th scope="col" class="th_120px">면적(㎡)</th>';
	$print_table .=  '									<th scope="col" class="th_120px">관리부서</th>';	
...
<div class="board_wrapper">
	<div class="module_list_box">
		...
		//	$category_parameter = make_GET_parameter($arr_temp);
			...
			$allType = true;
			echo print_category($data['category_1'], $category_1_list , $category_1_all, $arr_tempr, $_SERVER['PHP_SELF'], $allType);			
		}
		## -----------  카테고리 : end
...
//			print_r( $data['search_date'] );
			...
			## -----------  검색 영역 : start
			echo search_box_new($data['total_count'], $data['search_type'], $data['search_word'], $data['search_list'], $data['page_scale_search'], $data['page_scale'], $data['start_date'], $data['finish_date'], $data['search_parameter'],$data,false); 
			## -----------  검색 영역 : end
