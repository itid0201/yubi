	...
	***********/	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		if($buffer['photo_exist'] == 'true') {
			...
			$img = '<div class="img_box"><img src="/images/board/noimage_book_list.jpg" alt="이미지가 없습니다" width="324" height="324" /></div>';
		}	
...
//		$print_table_tr  .=  '    					<div class="hover_box"><div class="lt"></div><div class="lr"></div><div class="lb"></div><div class="ll"></div></div>';
		...
		$print_table_tr  .=  '    						<a href="'.(empty($buffer['link_url'])?'#none':$buffer['link_url']).'" '.(empty($buffer['link_url'])?'':' target="_blank" ').' data-idx="'.$buffer['idx'].'" data-link="'.$buffer['link_url'].'" title="'.$buffer['title'].'" class="target">';
		$print_table_tr  .= $img;
...
//		$print_table_tr  .=  '						<div class="bottom_btn"><a href="'.$href_view.'" class="modify">수정</a></div>';	
		$print_table_tr  .=  '						<div class="bottom_btn"><a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  title="'.$buffer['title'].'에 대한 글내용 보기." class="modify basic_cont">수정</a></div>';	
		}		
		$print_table_tr  .=  '						</div>';
		$print_table_tr  .=  '    				</div>';		
