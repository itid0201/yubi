...
//		$print_table_tr  .=  '								<td>'.$buffer['reg_date'].'</td>';		
		...
		if($data['use_list_attach'] == 'true') {
			...
			if($buffer['file_exist'] == 'true' || $buffer['photo_exist'] == 'true' || $buffer['movie_exist'] == 'true') {
				$print_table_tr  .= download_box_new("list", $buffer['idx'], $buffer['file_list']);			
			}
			$print_table_tr  .=  '							</td>';			
		}
		## ---------------------------------------------------------------
//		$print_table_tr  .=  '								<td>'.$buffer['visit_cnt'].'</td>';
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'담당부서, ':'').' 민원사무명, 서식으로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	if($data['use_category_1'] == 'true') 			$print_table .=  '	<th scope="col" class="th_200px">담당부서</th>';
	$print_table .=  '									<th scope="col">민원사무명</th>';
...
//	$print_table .=  '									<th scope="col" class="th_120px">등록일</th>';	
	if($data['use_list_attach'] == 'true') $print_table .=  '			<th scope="col" class="th_200px">서식</th>';
