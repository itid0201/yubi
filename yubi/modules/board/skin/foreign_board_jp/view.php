...
<div class="board_wrapper">
	<div class="module_view_box">
		<div class="view_titlebox">
			...
			<dl>
				<dt class="text_hidden">登録日</dt><dd><?php echo date('Y.m.d H:i',strtotime($data['reg_date'])); ?></dd>
				<dt>照会数</dt><dd><?php echo $data['visit_cnt']; ?></dd>
				<dt>登録者名</dt><dd><?php echo($data['writer_display'] == 'department' ? $data['depart_name'] : $data['reg_name']) ?></dd>
			</dl>
		</div>
		<div class="contbox">
			<?php  
		...
		## --------------------- 버튼 : start
		$print_button =  print_button_new_foreign('jp','view', $data['permission'], $data['parameter'], $url, $img_url, $user_info, $arr_data);
		...
		if( $data['use_pager'] == 'true' ){			
			...
			echo print_pager_foreign('jp', $data['nearby_articles'],$data['parameter'],$_SERVER['PHP_SELF']);
		}
		## ---------------------
