    ...
    <div class="board_wrapper">
        <div class="module_write_box">
            <form id="write_form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="selectForm"
                ...
                <input type="hidden" name="keyword_del" id="keyword_del"/> <input type="hidden" name="board_id"
                                                                                  id="board_id"
                                                                                  value="<?php echo $_SYSTEM['module_config']['board_id']; ?>"/>
                <input type="hidden" name="keyword" id="keyword" value=""/>


                <div class="title_write">
                    <label for="title">게시글 제목<span class="rd">(*)</span><span class="hid">필수입력</span></label>
                ...
                <div class="cont_write">
                    ...
                    <?php if ($_SYSTEM['module_config']['board_id'] == 'www_newsletter_req' || $_SYSTEM['module_config']['board_id'] == 'www_hope') { ?>
                    ...
                    <?php } else { ?>
                        ...
                        if ($data['use_lock'] == 'true') {
                            ...
                            <div class="write_box notice_box">
                                <div class="module_t">
                                    <label for="notice_check">공개여부<?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'open'); ?></label>
                                </div>
                                <div class="module_w">
                                    <input type="radio" name="open" id="open_allow"
                                           value="y" <?php echo $open_allow; ?> /><label for="open_allow">공개</label>&nbsp;&nbsp;&nbsp;
                                    <input type="radio" name="open" id="open_deny"
                                           value="n" <?php echo $open_deny; ?> /><label for="open_deny">비공개</label>
                                </div>
                            </div>
                        <?php } //if end ?>
                    <?php } ?>
                    ...
                    if ($data['use_top'] == 'true' && ($_SYSTEM['permission']['admin'] == 'true' || $_SYSTEM['permission']['manage'] == 'true') && in_array($data['mode'], array("write", "modify"))) {
                        ...
                        <div class="write_box date_select_box">
                            <div class="module_t">
                                <input type="checkbox" name="top" id="notice_check"
                                       value="y" <?php echo $top_checked; ?> />
                                <label for="notice_check">공지로 등록<span class="icon"></span></label></div>
                            <div class="module_w">
                                <div class="base_box date_box">
                                    <input type="hidden" id="top_start_hidden" name="top_start_hidden"
                                           value="<?php echo $data['top_start'] ?>"/>
                                    <label for="top_start" class="box_hidden">공지 시작시간</label>
                                    <input type="text" id="top_start" name="top_start" class="datetime datetime_notice"
                                           value="<?php echo str_replace("-", "", date('Y-m-d', strtotime($data['top_start']))); ?>" <?php echo empty($data['top_start']) ? ' placeholder="' . date('Ymd') . '"' : ''; ?>  <?php echo $top_disable; ?> />
                                    <a href="#none" class="date_icon"></a>
                                </div>
                                <span class="icon_text">~</span>
                                <div class="base_box date_box">
                                    <input type="hidden" id="top_end_hidden" name="top_end_hidden"
                                           value="<?php echo $data['top_end'] ?>"/>
                                    <label for="top_end" class="box_hidden">공지 종료시간</label>
                                    <input type="text" id="top_end" name="top_end" class="datetime datetime_notice"
                                           value="<?php echo str_replace("-", "", date('Y-m-d', strtotime($data['top_end']))); ?>" <?php echo empty($data['top_end']) ? ' placeholder="' . date('Ymd', strtotime("+7 day")) . '"' : ''; ?>  <?php echo $top_disable; ?> />
                                    <a href="#none" class="date_icon"></a>
                                </div>
                            </div>
                        </div>
                    ...
                    if ($data['use_category_1'] == 'true' && in_array($data['mode'], array("write", "modify"))) {
                        ...
                        $category_1 = '';
                        if (empty($data['category_1']) && !empty($_GET['category_1'])) $data['category_1'] = $_GET['category_1'];
                    ...
                    if ($data['use_tag'] == 'true' && !empty($data['tag_list_all'])) {
                        ...
                        <div class="write_box module_tegs">
                            <div class="module_t"><label for="category_1">
                                    <?php if ($_SYSTEM['module_config']['board_id'] == 'town_square_notice') { ?>
                                        읍면동 선택
                                    <?php } else { ?>
                                        tag 등록
                                    <?php } ?></label></div>
                    ...
                    <?php if ($_SYSTEM['module_config']['use_period'] == 'true') { ?>
                        ...
                        <div class="write_box date_select_box">
                            ...
                            <div class="module_w">
                                <div class="base_box date_box">
                                    <input title="게시 시작시간" type="hidden" id="period_start_hidden"
                                           name="period_start_hidden" value="<?php echo $data['period_start'] ?>"/>
                                    <label for="period_start" class="box_hidden">게시 시작시간</label>
                                    <input type="text" id="period_start" name="period_start"
                                           class="datetime datetime_normal"
                                           value="<?php echo str_replace("-", "", $data['period_start']); ?>" <?php echo empty($data['period_start']) ? ' placeholder="' . date('Ymd') . '"' : ''; ?> <?php echo $period_disable; ?> />
                                    <a href="#none" class="date_icon"></a>
                                </div>
                                <span class="icon_text">~</span>
                                <div class="base_box date_box">
                                    <input title="게시 종료시간" type="hidden" id="period_end_hidden" name="period_end_hidden"
                                           value="<?php echo $data['period_end'] ?>"/>
                                    <label for="period_end" class="box_hidden">게시 종료시간</label>
                                    <input type="text" id="period_end" name="period_end"
                                           class="datetime datetime_normal"
                                           value="<?php echo str_replace("-", "", $data['period_end']); ?>" <?php echo empty($data['period_end']) ? ' placeholder="' . date('Ymd', strtotime("+7 day")) . '"' : ''; ?> <?php echo $period_disable; ?> />
                                    <a href="#none" class="date_icon"></a>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    <div class="write_box">
                        <div class="module_t"><label
                                    for="reg_name"><?php echo $data['writer_display'] == 'department' ? '등록부서' : '등록자'; ?>
                                <span class="rd">(*)</span><span class="hid">필수입력</span></label></div>
                        <div class="module_w">
                            <?php if ($data['writer_display'] == 'department') { ?>
                                <input type="text" class="module_text _chk_input" name="depart_name" id="reg_name"
                                       value="<?php echo $data['depart_name']; ?>"/>
                            <?php } elseif ($data['writer_display'] == 'other_organ') { ?>
                                <input type="text" class="module_text _chk_input" name="reg_name" id="reg_name"
                    ...
                    } else if ($_SYSTEM['module_config']['board_id'] == 'youth_participate_budget_participation' || $_SYSTEM['module_config']['board_id'] == 'citizen_participate_budget_participation') {
                        echo '
												<div class="write_box">
													<div class="module_t">	
														<label for="varchar_1">연락처<em>(*)</em></label>
													</div>
													<div class="module_w">	
														<input type="text" name="varchar_1" id="varchar_1" class="module_text _chk_input" value="' . $data['varchar_1'] . '" />
													</div>
                    ...
                    <div class="write_contbox">
                        <div class="module_t" id="append_btn_zone">
                            <strong>내용<span class="rd">(*)</span><span class="hid">필수입력</span></strong><span>입력버튼을 클릭하면 아래에 입력박스가 생성됩니다.</span>
                            <ul class="append_btn">
                                ...
                                <!-- li class="image_append">
                                    <div class="cont_image_add" style="display:none;">
                                        <input type="file" title="파일첨부" id="cont_image" name="cont_image[]"
                                               data-width="<?php /*echo $_SYSTEM['module_config']['contents_img_width_size'];*/ ?>"/>
                                    </div>
                    ...
                    if ($_SYSTEM['module_config']['use_gallery_img'] == 'true') {
                        ...
                        <div class="write_box module_file">
                            <div class="module_t">
                                <?php if ($_SESSION['menu_log']['prev_menu_id'] == '11844') { ?>
                                    <strong>썸네일 이미지</strong>
                                <?php } else { ?>
                                    <strong>메인 이미지</strong>
                                <?php } ?>
                                <?php echo ($_SYSTEM['module_config']['use_gallery_img'] == 'true') ? '<span class="rd">(*)</span><span class="hid">필수입력</span>' : '' ?>
                            </div>
                            <div class="module_w">
                                <?php if ($_SESSION['menu_log']['prev_menu_id'] != '11844') { ?>
                    ...
                    <div class="write_box module_file">
                        <div class="module_t">
                            <strong>파일첨부</strong><?php echo ($_SYSTEM['module_config']['use_file_check'] == 'true') ? '<em>(*)</em>' : '' ?>
                        </div>
                        <div class="module_w">
                            <?php
                            //print_r($data['file_list']);
                            ## 갤러리 필수
                            $use_gallery_img = $use_file_check = false;
                            if ($_SYSTEM['module_config']['use_gallery_img'] == 'true') $use_gallery_img = true;
                            if ($_SYSTEM['module_config']['use_file_check'] == 'true') $use_file_check = 'true';

                            if ($data['file_upload_count'] > 0) echo upload_box_alt_tour($data['file_list'], $data['idx'], 0, $use_gallery_img, false, $data['file_upload_count'], '', '', $use_file_check);
                            ?>
                        </div>
                    </div>
                    <?php
                    ...
                    */
                    ...
                    // 220406 권재영 && $_SYSTEM['module_config']['use_logoff_write'] == 'true' 작성자 개인정보 동의를 위해 제거
                        ...
                        <div class="write_box module_privacy">
                            ...
                            <div class="module_w">
                                ...
                                <p>
                                    ...
                                    <input class="_chk_input" type="checkbox" name="agree_privacy" id="agree_privacy"
...
<?php if ($_SERVER['REMOTE_ADDR'] == '49.254.140.140' && $_SESSION['user_id'] == 'hya9196') { ?>
    <script src="<?php echo $_SERVER['SELF']; ?>js/write_new.js?build=<?php echo SERIAL; ?>"></script>
<?php } else { ?>
