...
//if( $_SERVER['REMOTE_ADDR'] == "49.254.140.140" || $_SERVER['REMOTE_ADDR'] == "168.131.244.65" ){
	if( $module->module_config['board_id'] == 'gongik_education_ing' ){
		if(date('Y-m-d', strtotime($buffer['varchar_1'])) > date('Y-m-d')) {
			$print_table_tr  .=  '    						<span class="wait">예정</span>';
		}elseif(date('Y-m-d', strtotime($buffer['varchar_1'])) <= date('Y-m-d') && date('Y-m-d', strtotime($buffer['varchar_2'])) >= date('Y-m-d')) {
			$print_table_tr  .=  '    						<span class="ing">진행중</span>';
		}elseif(date('Y-m-d', strtotime($buffer['varchar_2'])) < date('Y-m-d')) {
			$print_table_tr  .=  '    						<span class="end">마감</span>';
		} else {
			$print_table_tr  .=  '    						<span class="end">마감</span>';
		}
