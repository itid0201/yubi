<?php	
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 프로그램, '.$writer_name.', 성별'.($data['use_process_1'] == 'true'?', 처리상태':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?', 승인상태':'').($data['use_list_attach'] == 'true'?', 첨부파일':'').'로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	$print_table .=  '									<th scope="col">프로그램</th>';
	$print_table .=  '									<th scope="col" class="th_120px">'.$writer_name.'</th>';	
	$print_table .=  '									<th scope="col" class="th_100px">성별</th>';	
