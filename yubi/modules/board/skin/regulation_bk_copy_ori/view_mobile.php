	 ...
	 foreach($data['approval']['list'] as $list) {
		...
		//	$list['reply'] = preg_replace('/<(p|span|font)([^>]*)>/i','<\1>',$list['reply']);
			...
			echo '<div class="progress">
					...
					<ol>
						<li'.($list['process']=='reg'?' class="on"':'').'><span>신청'.($list['process']=='reg'?'(현재단계)':'').'</span></li>
						<li'.($list['process']=='accept'?' class="on"':'').'><span>접수'.($list['process']=='accept'?'(현재단계)':'').'</span></li>
						...
						<li'.($list['process']=='complete'?' class="on"':'').'><span>완료'.($list['process']=='complete'?'(현재단계)':'').'</span></li>
					</ol>	
				</div>
				<div class="answer">
					'.$list['reply'].'
