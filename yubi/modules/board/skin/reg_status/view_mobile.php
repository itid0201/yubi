	...
	if($_SYSTEM['module_config']['board_id'] == 'www_newsletter_req') {
		...
		if($_SYSTEM['permission']['manage'] == true || $_SYSTEM['permission']['admin'] == true) echo '<p class="birth">주소 : ('.$data['zipcode'].') '.$data['address_1'].' '.$data['address_2'].'</p>';
			...
			//echo $contents; 	
	  		...
	  		echo '<div class="item_viewbox wide_tit">';
	  
	  		echo $data['title'] == '' ? '' : '<div class="item_list"><span class="tit">관리번호</span><span class="cont">'.$data['title'].'</span></div>';
			echo $data['depart_name'] == '' ? '' : '<div class="item_list"><span class="tit">담당부서</span><span class="cont">'.$data['depart_name'].'</span></div>';	   
			echo $data['varchar_1'] == '' ? '' : '<div class="item_list"><span class="tit">상위법령</span><span class="cont">'.$data['varchar_1'].'</span></div>';
			  echo $data['varchar_2'] == '' ? '' : '<div class="item_list"><span class="tit">조례</span><span class="cont">'.$data['varchar_2'].'</span></div>';
			  echo $data['varchar_3'] == '' ? '' : '<div class="item_list"><span class="tit">규칙</span><span class="cont">'.$data['varchar_3'].'</span></div>';
			  echo $data['varchar_4'] == '' ? '' : '<div class="item_list"><span class="tit">훈령</span><span class="cont">'.$data['varchar_4'].'</span></div>';
			  echo $data['varchar_5'] == '' ? '' : '<div class="item_list"><span class="tit">예규</span><span class="cont">'.$data['varchar_5'].'</span></div>';
			  echo $data['varchar_6'] == '' ? '' : '<div class="item_list"><span class="tit">고시등</span><span class="cont">'.$data['varchar_6'].'</span></div>';
			  echo $data['varchar_7'] == '' ? '' : '<div class="item_list"><span class="tit">유형</span><span class="cont">'.$data['varchar_7'].'</span></div>';
			  echo $data['varchar_8'] == '' ? '' : '<div class="item_list"><span class="tit">등록사유</span><span class="cont">'.$data['varchar_8'].'</span></div>';
			  echo $data['varchar_9'] == '' ? '' : '<div class="item_list"><span class="tit">법령공표일</span><span class="cont">'.$data['varchar_9'].'</span></div>';
			  echo $data['varchar_10'] == '' ? '' : '<div class="item_list"><span class="tit">규칙</span><span class="cont">'.$data['varchar_10'].'</span></div>';	  
			  echo $data['varchar_11'] == '' ? '' : '<div class="item_list"><span class="tit">존속기한</span><span class="cont">'.$data['varchar_11'].'</span></div>';
			  echo $data['varchar_12'] == '' ? '' : '<div class="item_list"><span class="tit">규제시행</br>폐지일</span><span class="cont">'.$data['varchar_12'].'</span></div>';
			  echo $data['contents_en'] == '' ? '' : '<div class="item_list"><span class="tit">규제목적</span><span class="cont">'.$data['contents_en'].'</span></div>';
			  echo $data['contents_jp'] == '' ? '' : '<div class="item_list"><span class="tit">규제내용</span><span class="cont">'.$data['contents_jp'].'</span></div>';
			  echo $data['varchar_15'] == '' ? '' : '<div class="item_list"><span class="tit">등록후변동결과</span><span class="cont">'.$data['varchar_15'].'</span></div>';
	  		  echo '</div>';					
			echo '</div>';	  
    ...
    <form>
      <div class="comment_input_box"><strong class="title_box"><span class="icon"></span>댓글(12)</strong><!--숫자는 댓글 카운트수로-->
        <div class="comment_input_inner">
          <div for="comment_input">댓글은 실명인증후 작성하실 수 있습니다.</div>
