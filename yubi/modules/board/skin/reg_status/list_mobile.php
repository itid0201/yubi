	...
	***********/	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr = '<div class="item">
									<a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  title="'.$buffer['contents'].'에 대한 글내용 보기."  '.$link_class.'>
										<strong>'.($buffer['is_top']=='true' ? $top_img:'').$buffer['contents'].$print_img.'</strong>
										<span class="date">'.date('Y-m-d',strtotime($buffer['reg_date'])).'</span>
									</a>
							</div>';
		$original_cnt++;
...
<div class="board_wrapper">
	<div class="module_list_box">
		...
		<div class="board_list_box">
			...
			echo search_box_new($data['total_count'], $data['search_type'], $data['search_word'], $type, $data['page_scale_search'], $data['page_scale'], $data['start_date'], $data['finish_date'], $data['search_parameter'], '', false); 
			## -----------  검색 영역 : end
