...
//echo '<pre>';print_r( $data );exit;
	...
	for($iter=0 ; $iter < $data['count'] ; $iter++) {
		...
		foreach($buffer['photo_list'] as $item) {
			/*20230907 권재영 불필요한 대체 텍스트로 인해 alt를 모두 비우고 a태그에 title로 추가 */
			$img_list[] = array('file_name'=>$item['photo_name'],'alt'=>"",'idx'=>$buffer['idx']);
			//$img_list[] = array('file_name'=>$item['photo_name'],'alt'=>$item['photo_alt'],'idx'=>$buffer['idx']);
		}
	
if($_SERVER['REMOTE_ADDR'] == '49.254.140.140'  && $_SESSION['user_id'] == 'csi4613'){
   # echo '<pre>'; print_r(get_included_files()); exit;
}
		...
		if( $img_count > 0 ){
			...
			##2022.08.03 최무성 여수 접근성 작업, a 안에 제목 없으면 이미지 알트는 "글 제목"
			$img = $module->get_munti_thumb_new_alt($width,$height,$margin,$img_list,$buffer['photo_path'],$href_view,$img_count,$buffer['title']);	
		
			
			
		}else{
			$img = '<div class="img_box"><img src="/images/board/noimage_photo.jpg" alt="" /></div>';
		}
		...
		$print_table_tr  .=  '    						<a href="'.$href_view.'" class="item_click">';
		$print_table_tr  .= $img;
		$print_table_tr  .=  '    						<div class="tit_box">';
		$print_table_tr  .=  '    							<strong>'.$buffer['title'].$print_img.'</strong>';
		$print_table_tr  .=  '								<dl>';
		...
		$print_table_tr  .=  '    											<li class="facebook share_btn"><a href="#none" class="share_btn" data-type="facebook" data-url="'.$href_view.'" data-title="'.$buffer['title'].'" title="새창"><span class="icon share_btn">페이스북</span></a></li>';
		$print_table_tr  .=  '    											<li class="twitter share_btn"><a href="#none" class="share_btn" data-type="twitter" data-url="'.$href_view.'" data-title="'.$buffer['title'].'" title="새창"><span class="icon  share_btn">엑스</span></a></li>';
		//$print_table_tr  .=  '    											<li class="kakaostory share_btn"><a href="#none" class="share_btn" data-type="kakaostory" data-url="'.$href_view.'" data-title="'.$buffer['title'].'" title="새창"><span class="icon  share_btn">카카오스토리</span></a></li>';
		$print_table_tr  .=  '    											<li class="band share_btn"><a href="#none" class="share_btn" data-type="band" data-url="'.$href_view.'" data-title="'.$buffer['title'].'" title="새창"><span class="icon  share_btn">네이버밴드</span></a></li>';
		$print_table_tr  .=  '    										</ul>';
		$print_table_tr  .=  '    									</div>';
		$print_table_tr  .=  '    								</li>';
		$print_table_tr  .=  '    							</ul>';
