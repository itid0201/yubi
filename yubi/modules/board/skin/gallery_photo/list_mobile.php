<?php	
	...
	***********/	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		if(empty($buffer['photo_list'])){
			$img_list[] = array('file_name'=>$buffer['photo_name'],'alt'=>$buffer['photo_alt'],'idx'=>$buffer['idx']);
			$buffer['photo_cnt'] = !empty($buffer['photo_name'])?1:0;
		}
		...
		if( $img_count > 0 ){
			...
			$img = '<div class="img_box"><img src="/images/board/noimage_photo.jpg" alt="이미지가 없습니다" /></div>';
		}
if($_SERVER['REMOTE_ADDR'] == '49.254.140.140'  && $_SESSION['user_id'] == 'siha1997' && $buffer['idx'] == '795'){
...
//		$print_table_tr  .=  '    					<div class="hover_box"><div class="lt"></div><div class="lr"></div><div class="lb"></div><div class="ll"></div></div>';
		...
		$print_table_tr  .=  '    						<a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  class="item_click item_cont" title="'.$buffer['title'].'">';
		$print_table_tr  .= $img;
		$print_table_tr  .=  '    						</a>';
