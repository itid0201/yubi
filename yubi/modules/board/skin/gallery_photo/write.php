							...
							<div class="module_write_box">
								<form id="write_form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="selectForm" enctype="multipart/form-data">
									...
									<div class="cont_write">
										...
										<?php 
										/*if($_SYSTEM['module_config']['board_id'] == 'noin_photo' && $mode == "modify" ) {	
										?>
										<div class="write_box">
											<div class="module_t"><label for="reg_name">작성일</label></div>
											<div class="module_w">												
												<div class="base_box date_box">
													<label for="reg_date" class="box_hidden">게시 종료시간</label>
													<input type="text" id="reg_date" name="reg_date" class="datetime datetime_normal" value="<?php echo date('Ymd',strtotime($data['reg_date']))?>" />
													<a href="#none" class="date_icon"></a>
												</div>
											</div>
										</div>
