...
<fieldset>
	<legend class="write_label">Mandatory</legend>
		...
		?>
		<div class="mob">
            <label>Publishes<?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'open'); ?></label>
		...
		<div class="mob">
        <label for="title">Title<em>(*)</em><?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'title'); ?></label>
                <?php 
                ...
                ?>
                <input type="text" class="w95" name="title" id="title" value="<?php echo $data['title'];?>" data-clear-btn="true" />
            </div>
		<div class="mob">
        		<label for="reg_name">Writer<em>(*)</em><?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'open'); ?></label>

				<?php if($data['writer_display'] == 'department') { ?>
                <input type="text"  name="depart_name" id="reg_name" value="<?php echo $data['depart_name']; ?>" />
                <?php } elseif($data['writer_display'] == 'other_organ') { ?>
                <input type="text"  name="reg_name" id="reg_name" value="<?php echo $data['reg_name']; ?>" />
                <?php } else { ?>
                <input type="text"  name="reg_name" id="reg_name" value="<?php echo $data['reg_name']; ?>" <?php echo ($data['read_only_reg_name'] == 'true' ? 'readonly="readonly"' : '')?> />
                <?php } ?>
	...
	<?php
		...
		?>
        <div class="mob"><label for="category_1">Category<?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'category_1'); ?></label>
		...
		<div class="mob">
        	<label>Phone Number</label>
				...
				?>
				<input type="text" id="phone_1" maxlength="5" size="5" name="phone_1" value="<?php echo $phone_1;?>" title="회원 연락처 첫자리"/> - 
				<input type="text" id="phone_2" maxlength="4" size="5" name="phone_2" value="<?php echo $phone_2;?>" title="회원 연락처 중간자리"/> - 
				<input type="text" id="phone_3" maxlength="4" size="5" name="phone_3" value="<?php echo $phone_3;?>" title="회원 연락처 마지막자리" />
		...
		<div class="mob">
        	<label for="reply_sms_recv_number">SMS Phone Number</label>
			<input type="text" id="reply_sms_recv_number" maxlength="14" size="14" name="reply_sms_recv_number" value="<?php echo $data['reply_sms_recv_number'];?>" title="SMS Phone Number" />

			</div>
		<?php }?>
        
		<div class="mob">
        <label for="contents">Contents<em>(*)</em><?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'contents'); ?></label>
			<textarea name="contents" id="contents" rows="25" cols="70"><?php echo $data['contents'];?></textarea>
		</div>
        
        </fieldset>

<fieldset>
	<legend class="write_label">Additional Input</legend>
		...
		<div class="mob">
            <label for="passwd">Password<?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'passwd'); ?></label>
            <input type="password"  size="50" name="passwd" id="passwd" value="" />
         </div>
		<div class="mob">
            <label for="search_tag">Key Code<?php check_required_field_wirte_mode($_SYSTEM['module_config']['required_value'], 'text_keycode'); ?></label>
		...
		?>
		<div class="mob">
            <label for="file_<?php echo $iter;?>">Attached Files<?php echo($iter+1);?></label>
...
<?php 
	...
	if($_SYSTEM['module_config']['use_privacy_write'] == 'true' ){ ## 개인정보이용동의체크시
		...
		echo print_button_mobile_eng('write', $data['permission'], $data['parameter'], $url, $img_url, $user_info, $arr_data);
	}
?>
</form>
<?php 
