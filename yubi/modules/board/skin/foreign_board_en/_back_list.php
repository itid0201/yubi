...
/*	echo '  <colgroup>';
	...
	echo '      <th scope="col" class="w45">Title</th>';
	if($data['use_process_1'] == 'true') echo '<th scope="col">Processing</th>';
	if($data['use_allow'] == 'true') echo '<th scope="col">Status</th>';
	echo '      <th scope="col" class="w15">Writer</th>';
	if($data['use_reg_date'] != 'false') echo '      <th scope="col">Posted at</th>';
	echo '      <th scope="col" class="bg_none">Hits</th>';
	echo '    </tr>';
	echo '  </thead>';
	echo '  <tbody>';
	...
	if($iter == 0) echo '<tr><td colspan="'.$colspan.'" class="list_empty">There is no search posts.</td></tr>';
...
//	echo '</form>';
	...
	echo print_button_tour_eng('list', $data['permission'], $data['parameter'], $url, $img_url, $user_info, $data['use_logoff_write']);
