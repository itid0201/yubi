...
//	$hidden_img = '<span class="icon_hidden">비승인글</span>'; 	
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr  .=  '								<td>'.$buffer['email'].'</td>';
		//$print_table_tr  .=  '								<td><a href="'.$href_view.'" '.$link_class.' title="'.$buffer['title'].' 에 대한 글내용 보기." >'.$writer.$print_img.'</td></a>';
		$print_table_tr  .=  '								<td><div '.$link_class.'>'.$writer.$print_img.'</div></td>';
		$print_table_tr  .=  '								<td>'.$buffer['phone'].'</td>';
		...
		$cancel_date = (date('Y-m-d')==date('Y-m-d',strtotime($buffer['varchar_7']))?substr($buffer['varchar_7'],-8):substr($buffer['varchar_7'],0,10));
		$print_table_tr  .=  '								<td>'.$cancel_date.'</td>';		
		...
		$str_cate = '';
		$str_cate .= '<option value="신청" name="신청" '.($buffer['category_1']=='신청'?' selected="selected"':'').'>신청</option>';
		$str_cate .= '<option value="해지" name="해지" '.($buffer['category_1']=='해지'?' selected="selected"':'').'>해지</option>';
		
		$print_table_tr  .=  '<td class="list_cate"><select class="change_cate" name="'.$buffer['reg_name'].'" id="idx_'.$buffer['idx'].'" title="'.$buffer['board_id'].'">'.$str_cate.'</select></td>';	
		$print_table_tr  .=  '							</tr>';
	}	
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 제목, '.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').' 첨부파일, '.$writer_name.', 신청일자, 해지일자, 상태로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
	...
	$print_table .=  '									<th scope="col">이메일</th>';
	$print_table .=  '									<th scope="col">'.$writer_name.'</th>';
	$print_table .=  '									<th scope="col">전화번호</th>';
	...
	$print_table .=  '									<th scope="col" class="th_120px">신청일자</th>';	
	$print_table .=  '									<th scope="col" class="th_120px">해지일자</th>';	
...
<div class="board_wrapper">
	<div class="module_list_box">
		...
		<div class="board_list_box">
			...
			## index의 print_excel 함수 없어서 오류나고 있음. class에 함수 만들어서 기능구현해주세요. ( 지영씨 )
			## 2020/ 06.24 서희진 => 이메세지는 작업후에 삭제해주세용~~~~~~~~~~~~~~~~~
			//echo '<div class="newsletter_btn"><a href="?mode=excel" ><span class="btn excel">엑셀출력</span></a></div>';			
...
$(function() { 
	...
	$(".change_cate").on("change",function(){
		...
		var mode = $(this).val();
		if (confirm(name+"님의 신청 정보를 "+mode+"(으)로 변경하시겠습니까?") == true){ 
			var idx = $(this).attr("id").replace("idx_","");
			var request = $.ajax({
				url: "/ybscript.io/apply/change",
				method: "POST",
				data: { idx : idx, mode : mode, board_id : board_id },
				dataType: "html"
			});
