<?php	
	...
	header( "Content-Disposition: attachment; filename = ".$_SYSTEM['menu_info']['title'].".xls" );   
...
//		$print_table_tr  .=  '								</td>';
		...
		$print_table_tr  .=  '								<td>'.$buffer['varchar_4'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['title'].'</td>';
		$print_table_tr  .=  '								<td>'.$buffer['zipcode'].' '.$buffer['address_1'].' '.$buffer['address_2'].'</td>';
...
//		$print_table_tr  .=  '								<td>'.$buffer['visit_cnt'].'</td>';
	...
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 성명, 서비스업종, 주소, 사업자등록번호, 연락처, '.($data['use_process_1'] == 'true'?'처리상태, ':'').' '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').($data['use_list_attach'] == 'true'?'첨부파일,':'').', 등록일로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
...
//	$print_table .=  '									<th scope="col">제목</th>';
	$print_table .=  '									<th scope="col" class="th_120px">성명</th>';
	$print_table .=  '									<th scope="col">서비스업종</th>';	
	$print_table .=  '									<th scope="col">업종구분</th>';
	$print_table .=  '									<th scope="col">상호명</th>';
	$print_table .=  '									<th scope="col">주소</th>';
	$print_table .=  '									<th scope="col">사업자등록번호</th>';
	$print_table .=  '									<th scope="col">연락처</th>';
