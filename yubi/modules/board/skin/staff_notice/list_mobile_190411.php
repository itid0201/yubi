	...
	***********/
	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		if( $buffer['is_top'] == 'true' ){	// top 설정
			...
			if($buffer['comment_cnt'] > 0) {
				$str_list .= '<span class="comment_cnt"><span>댓글</span>'.$buffer['comment_cnt'].'</span>';
			}
			...
			$str_list .= '				<span class="'.$stat_class[$buffer['process_1']].'">'.$buffer['process_1'].'</span> ';
			}*/
			...
			$str_list .= '                 <p class="date_wrap"><span class="date"> '.$buffer['reg_date'].' </span>';
			...
			if($data['use_process_1'] == 'true') {
				if($buffer['process_1'] == '신청') {
					$str_list .= '<td><div class="proc_wrap"><span class="request">'.$buffer['process_1'].'</span></div></td>';
				} elseif($buffer['process_1'] == '접수') {
					$str_list .= '<td><div class="proc_wrap"><span class="receipt">'.$buffer['process_1'].'</span></div></td>';
				} elseif($buffer['process_1'] == '보류') {
					$str_list .= '<td><div class="proc_wrap"><span class="defer">'.$buffer['process_1'].'</span></div></td>';
				} elseif($buffer['process_1'] == '완료') {
					$str_list .= '<td><div class="proc_wrap"><span class="complete">'.$buffer['process_1'].'</span></div></td>';
				}
			}
			$str_list .= '			</div>				 ';
			##-- 승인 비승인 / process ------------ E
...
<script type="text/javascript">
	...
	jQuery.fn.center = function () {
		this.css("top", Math.max(0, (($(window).height() - $(this).outerHeight()) / 2) + $(window).scrollTop() - 276) + "px");
