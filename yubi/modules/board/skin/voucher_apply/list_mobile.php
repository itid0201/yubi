	...
	for($iter=0 ; $iter<$data['count'] ; $iter++) {
		...
		$print_table_tr = '<div class="item">
									<a href="#none" data-idx="'.$buffer['idx'].'" data-link="'.$href_view.'"  title="'.$buffer['title'].'에 대한 글내용 보기."  '.$link_class.'>
										<strong>'.($buffer['is_top']=='true' ? $top_img:'').$tag_span.(($buffer['board_id']=='www_notice' && !empty($buffer['depart_name']))?'['.$buffer['depart_name'].'] ':'').$buffer['title'].$print_img.'</strong>
										
										<span class="list_writer">'.$writer.'</span>
										<span class="list_cate">'.$buffer['category_1'].'</span>
