  ...
  <div class="contbox">
	...
	if($_SYSTEM['module_config']['board_id'] == 'www_newsletter_req') {
		...
		if($_SYSTEM['permission']['manage'] == true || $_SYSTEM['permission']['admin'] == true) echo '<p class="birth">주소 : ('.$data['zipcode'].') '.$data['address_1'].' '.$data['address_2'].'</p>';
			...
			echo '<ul>
					<li><span class="subt">연락처</span>'.$data['phone'].'</li>';
			echo '<li><span class="subt">이메일</span>'.$data['email'].'</li>';
			echo '<li><span class="subt">주소</span>('.$data['zipcode'].')'.$data['address_1'].$data['address_2'].'</li>';
			echo '<li><span class="subt">지역구분</span>'.$data['varchar_2'].'</li>';
			echo '<li><span class="subt">분류</span>'.$data['varchar_3'].'</li>';
			if(!empty($data['varchar_5'])) echo '<li><span class="subt">정기모임</span>'.$data['varchar_5'].'</li>';
			if(!empty($data['varchar_6'])) echo '<li><span class="subt">정기모임장소</span>'.$data['varchar_6'].'</li>';
			//if(!empty($data['varchar_7'])) echo '<li><span class="subt">창립일</span>'.$data['varchar_7'].'</li>';
			if(!empty($data['varchar_4'])) echo '<li><span class="subt">회원가입조건</span>'.$data['varchar_4'].'</li>';
