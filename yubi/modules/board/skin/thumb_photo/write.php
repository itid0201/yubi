						...
						<div class="board_wrapper">
							<div class="module_write_box">
								<form id="write_form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="selectForm" enctype="multipart/form-data">
									...
									<?php 
									## 카테고리를 사용하지만 카테고리 탭 사용없이 메뉴 POST값으로 category_1  값을 받을떄 사용. 
									if($data['use_category_1'] == 'true' && $_SYSTEM['module_config']['use_category_tab'] != "true" ){
										echo '<input type="hidden" name="category_1" value="'.$_REQUEST['category_1'].'" />';
									}
									?>
									...
									<div class="cont_write">
										...
										##카테고리
										if($data['use_category_1'] == 'true' && $_SYSTEM['module_config']['use_category_tab'] == "true" && in_array($data['mode'],array("write","modify") ) ){	
										...
										<?php if($_SERVER['PHP_SELF'] == '/www/community/portfolio/portfolio_kiosk'){ ?>
										<div class="write_box" id="type_box" <?php echo ($data['category_1'] == '포토키오스크'? 'style="display: block;"' : 'style="display: none;"') ?> >
											<div class="module_t"><label for="product_type">제품 분류</label></div>
											<div class="module_w">
												<input type="hidden" id="varchar_5" name="varchar_5" value="<?php echo $data['varchar_5']; ?>" />
												<input type="radio" class="transparent_box" name="product_type" id="product_type1" <?php echo ($data['varchar_5'] == '스탠드'? 'checked="checked"' : ''); ?> value="스탠드"/><label for="product_type1">스탠드</label>
												<input type="radio" class="transparent_box" name="product_type" id="product_type2" <?php echo ($data['varchar_5'] == '듀얼'? 'checked="checked"' : ''); ?> value="듀얼"/><label for="product_type2">듀얼</label>
												<input type="radio" class="transparent_box" name="product_type" id="product_type3" <?php echo ($data['varchar_5'] == '크로마키'? 'checked="checked"' : ''); ?> value="크로마키"/><label for="product_type3">크로마키</label>
											</div>
										</div>
										<?php } ?>
										...
										<?php if($_SERVER['PHP_SELF'] == '/www/community/portfolio/portfolio_kiosk'){ ?>
										<div class="write_box">
											<div class="module_t"><label for="product_info">제품 정보</label></div>
											<div class="module_w">
												<input type="text" class="transparent_box" name="varchar_4" id="product_info" value="<?php echo $data['varchar_4']; ?>" placeholder="국가명/도시명/기기명/브랜드명"/>
											</div>
										</div>

										<?php }elseif($_SERVER['PHP_SELF'] == '/www/community/portfolio/portfolio_web'){ ?>
										<div class="write_box">
											<div class="module_t"><label for="url_info">URL</label></div>
											<div class="module_w">
												<input type="text" class="transparent_box" name="varchar_2" id="url_info" value="<?php echo $data['varchar_2']; ?>" placeholder="http://www.yubi.co.kr/"/>
											</div>
										</div>
										<?php } ?>
										...
										?>		
										<?php
										## 2021.06.02 서희진 : 메인이미지 사용은 모듈설정값으로 한다.										
										//if( $_SYSTEM['module_config']['skin_style'] == 'thumb_photo' ){
										if( $_SYSTEM['module_config']['main_image_use'] == 'true' ){
										?>
										...
										?>
										<div class="write_box module_file">
											<div class="module_t"><strong>파일첨부</strong><?php echo ( $_SYSTEM['module_config']['use_file_rield'] == 'true')?'<em>(*)</em>':''?></div>
											<div class="module_w">												
												<?php 
												//print_r( $_SYSTEM['module_config']['ext_photo_file'] );
												 ## 갤러리 필수
...
<script>
	...
	$("input:radio[name='product_type']").on('click', function () {
      $("#varchar_5").val($(this).val());
    })

	$("#category_1").on('change', function () {
		if($("#category_1").val() != '포토키오스크'){
			$("#type_box").hide();
			$("input:radio[name='product_type']").prop('checked',false);
			$("#varchar_5").val('');
		}else{
			$("#type_box").show();
		}
