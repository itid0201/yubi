...
for ($iter = 0; $iter < $data['count']; $iter++) {
  ...
  if ($buffer['photo_exist'] == 'true') {
    $img = '<div class="thumb_box"><img src="' . $buffer['photo_path'] . '430x263x'.$_SYSTEM['module_thumbnail_quality'] . $buffer['photo_name'] . '" alt="' . htmlspecialchars($buffer['photo_alt']) . '" /></div>';
    $link_class .= " hasthumb";
  } else {
  ...
  if ($img_count > 0) {
    ...
    $img = '<div class="thumb_box"><img src="' . $buffer['photo_path'] . '430x263/' . $buffer['photo_name'] . '" alt="' . htmlspecialchars($buffer['photo_alt']) . '" /></div>';
  } else {
    $img = '<img src="/images/board/noimage_thumb_photo.png" alt="이미지가 없습니다" />';
  }
  ...
  if(($_SERVER['PHP_SELF'] == '/www/community/portfolio/portfolio_web') && ($_SYSTEM['permission']['admin'] == true)){
     $print_table_tr .= '    			<a href="' . $href_view . '" class="item_click">상세보기</a></p>';
  }
  ...
  if($_SERVER['PHP_SELF'] == '/www/community/portfolio/portfolio_web'){
    $print_table_tr .= '    						<a href="' . $buffer['varchar_2'] . '" target="_blank" class="item_click">';
  }else{
