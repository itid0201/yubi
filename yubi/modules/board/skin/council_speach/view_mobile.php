...
<div class="module_view_box">
  <div class="view_titlebox">
    ...
    <span>
	<?php //20210114 김다정 나주도시재생 보도자료 reg_date 수정
		echo ($_SYSTEM['module_config']['board_id'] == 'njursc_newsrelease'?date("Y-m-d", strtotime($data['varchar_1'])):date('Y.m.d ',strtotime($data['reg_date']))) ; 
	?> / 
	<?php echo($data['writer_display'] == 'department' ? $data['depart_name'] : $data['reg_name']); ?> / 
	<?php echo '담당부서 : '.$data['depart_name']; ?>
	</span></div>
  ...
  <div class="contbox">
	...
	if($_SYSTEM['module_config']['board_id'] == 'www_newsletter_req') {
		...
		if($_SYSTEM['permission']['manage'] == true || $_SYSTEM['permission']['admin'] == true) echo '<p class="birth">주소 : ('.$data['zipcode'].') '.$data['address_1'].' '.$data['address_2'].'</p>';
			...
			## -------------- 콘텐츠
			echo '<p class="view_board_tit01">의회발언</p>';
	  		echo '<div>'.str_replace('< ','&lt;&nbsp;',$contents).'</div>';
			
	  		echo '<p class="view_board_tit02">시입장</p>';				
			echo '<div>'.str_replace('< ','&lt;&nbsp;',$contents_en).'</div>';
