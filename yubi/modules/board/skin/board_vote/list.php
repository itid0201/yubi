<?php	
	...
	$print_table  = '<p class="btn_p align_right"><a class="p2 mar10" href="/www/support/vote/">명부조회 바로가기</a></p>';	
	$print_table .=  '						<table class="board_basic">';
	$print_table .=  '  							<caption>'.$_SYSTEM['menu_info']['title'].' 게시물. 총 '.$data['total_count'].'건, '.$data['total_page'].'페이지 중 '.$data['page'].'페이지 '.$data['count'].'건 입니다. 본 데이터표는 '.$colspan.'컬럼, '.$data['count'].'로우로 구성되어 있습니다. 각 로우는 번호, '.($data['use_category_1'] == 'true'?'분류, ':'').' 제목,  '.($data['use_allow'] == 'true' && $_SYSTEM['permission']['admin'] == 'true'?'승인상태, ':'').($data['use_list_attach'] == 'true'?'첨부파일,':'').$writer_name.', '.($data['use_process_1'] == 'true'?'처리상태, ':'').' 등록일, 조회수로 구성되어 있습니다.</caption>';
	$print_table .=  '							<thead>';
	$print_table .=  '								<tr>';
